...
test_that(".update_dimension_date on Postgres", {
  if (!identical(Sys.getenv("CI_Postgres"), "true")) {
    skip("CI_Postgres is set to false")
  }
...
test_that("Ramses on PosgreSQL (system test)", {
  ...
  expect_true(bridge_episode_therapy_overlap(pq_conn))
  expect_error(bridge_episode_therapy_overlap(pq_conn))
  expect_true(bridge_episode_therapy_overlap(pq_conn, overwrite = TRUE))
  test_bridge_th_overlap <- tbl(pq_conn, "bridge_episode_therapy_overlap") %>% 
    dplyr::filter(encounter_id == "3968305736") %>% 
    dplyr::collect() %>% 
  ...
  expect_true(bridge_drug_prescriptions_date(pq_conn))
  expect_error(bridge_drug_prescriptions_date(pq_conn))
  expect_true(bridge_drug_prescriptions_date(pq_conn, overwrite = TRUE))
  test_bridge_rx_date <- tbl(
    pq_conn,
    "bridge_drug_prescriptions_date") %>% 
  ...
  expect_is(
    therapy_timeline(MedicationRequest(conn = pq_conn,
                                       id = "4d611fc8886c23ab047ad5f74e5080d7")), 
    "timevis")
...
test_that("Encounter class on Postgres", {
  ...
  last_temp_2encounters <- clinical_feature_last(
    Encounter(pq_conn, c("3968305736", "9278078393")),
    observation_code = "8310-5",
    hours = 24
  ) %>% 
    longitudinal_table(collect = T)
