...
Several functions are available to enrich the therapy table with fields characterising the clinical
state and trajectory of the patient. For instance, we may be interested in temperature: 
...
- `clinical_feature_range()` computes the number of observations falling both within and outside a specified range (eg 36°C--38°C); or lying below/above a specified threshold (eg 38°C); enabling to classify the patient as hypothermic/afebrile/febrile,
...
clinical_feature_range(
  ...
  hours = 6
) %>% therapy_table(collect = TRUE) %>% 
  mutate(
    range_temperature_N = range_temperature36_38_6h_strictly_under + 
      range_temperature36_38_6h_in_range + 
      range_temperature36_38_6h_strictly_over
  ) %>% 
  mutate(
    febrile = case_when(
      range_temperature36_38_6h_strictly_over > range_temperature_N * .333 ~ "febrile",
      range_temperature36_38_6h_in_range > range_temperature_N * .333 ~ "afebrile",
      range_temperature36_38_6h_strictly_under > range_temperature_N * .333 ~ "hypothermic",
      TRUE ~ NA_character_
    )
  ) %>% 
  select(t, t_start, febrile, starts_with("range_temperature")) %>% 
  filter(t >= 13)
```

The example below demonstrates how to track the blood neutrophil count (an important measure that the patient is improving after treatment is initiated) is going up or down. The `hours` parameter is set 48 hours to capture two recent values and ensure a slope can be computed.

We can see that at the time of therapy initiation, the neutrophil count was increasing by an average 0.09 10<sup>9</sup>/L every hour (neutrophilia). After 80 hours (day 3), we see first evidence of the neutrophil going down at a rate of 0.09 10<sup>9</sup>/L every hour. This suggests the infection is under control.
...
clinical_feature_ols_trend(
  ...
  select(t, t_start, ols_neutrophils_48h_slope, ols_neutrophils_48h_intercept) %>%
