...
- `clinical_feature_range()` computes the number of observations falling within and outside a specified range (eg 36°C--38°C) or lying below/above a specified threshold (eg 38°C).
...
Another example below tracks the blood neutrophil count, an  important measure to determine whether the patient is getting better after treatment is initiated. We want to know whether this parameter is going up, or down. The `hours` parameter is set to 48 hours: the function will look at all neutrophil counts in the last 48 hours. This should ensure we capture two recent values and can compute a slope.
