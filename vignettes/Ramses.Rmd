...
length_therapy_by_spell %>% 
  ...
  summarise(
    `Total admissions` = sum(1),
    `Total admissions with therapy` = sum(!is.na(LOT)),
    `Mean LOT` = round(mean(LOT, na.rm = T), 1),
    `Median LOT` = round(median(LOT, na.rm = T), 1),
    percentile25 = round(quantile(x = LOT, probs = .25, na.rm = T), 1),
    percentile75 = round(quantile(x = LOT, probs = .75, na.rm = T), 1)
    ) %>% 
  transmute(
...
consumption_aware_episodes_denom <- study_pop %>% 
  ...
  summarise(bed_days =  sum(ifNull(ramses_bed_days, 0.0))) %>% 
  collect()
