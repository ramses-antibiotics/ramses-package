...
`r formatC(length(unique(inpatient_episodes$spell_id)), big.mark = ",")`
...
For the purposes of this vignette, one may use on a small [SQLite
3](https://www.sqlite.org) database on the local hard drive. This
...
We begin with prescription records (but you can choose to start with
other records if you prefer).
...
We first map drug concepts thanks to the `as.ab()` function provided by
...
Using the AMR package's antibiotic codes gives access to its wide range
...
With the AMR package, it is possible to infer important properties of an
antibacterial/antifungal drug:

-   its ATC class and group, which can be useful for instance in order
    to identify antifungals (group 'Antifungals/antimycotics')
...
Another look-up table `reference_drug_frequency` available within Ramses
is also used to convert the `frequency` character variable into a
numeric variable indicating the daily frequency of administration.
...
and transfers. Ramses is designed around the notion of *hospital spells*
identified by variable `spell_id`, which correspond to the window from
the time a patient is admitted to the time a patient is discharged.

In addition, Ramses use subdivisions of *hospital spells* called
...
spells* consist of at least one *episode of care*. Episodes of care are
...
Hospital spells and episodes of care are recorded in a single table in
...
-   \link[common infections and indications]{antibiotic_icd_indications}
...
be found in `reference_specimen_type`.
...
\code{\link[AMR]{mdro}()} provides classification based on:
