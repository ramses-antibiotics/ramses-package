...
We can now begin validating and loading records into the database. This may be done in any order.

# Medication records

## Drug prescriptions

We begin with prescription records. 
...
Data can now be loaded into the database
...
One particular set of validation constraints is that units conform to the \href{https://ucum.org}{Unified Code for Units of Measure (UCUM)}. Units are implemented using the `units` package in R. Yet, many important units in medicine are not listed in the UCUM. For instance: 
